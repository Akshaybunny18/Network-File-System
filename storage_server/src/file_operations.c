...
WriteContext *begin_write(FileOperationContext *ctx, const char *filepath, int sentence_index, const char *username, int *error_code)
{
    ...
    wctx->word_index_offset = 0; // Initialize offset tracking
    ...
    wctx->original_sentence_index = sentence_index; // Track original for lock release
...
int write_word(WriteContext *wctx, int word_index, const char *content)
{
    ...
    // Apply word index offset (adjusted after sentence splits)
    int adjusted_index = word_index - wctx->word_index_offset;
    if (adjusted_index < 0)
    {
        adjusted_index = 0;
    }
    ...
    if (adjusted_index < 0 || adjusted_index > sentence->word_count)
    {
        log_message(ss_logger, LOG_WARN, "Adjusted word index %d (original %d, offset %d) out of range (max: %d)",
                    adjusted_index, word_index, wctx->word_index_offset, sentence->word_count);
    ...
    if (adjusted_index == sentence->word_count && sentence->delimiter != '\0')
    {
        ...
        // Update context to point to new sentence and adjust offset
        wctx->sentence_index = wctx->sentence_index + 1;
        wctx->word_index_offset = word_index; // Remember where we split

        // We are inserting at the beginning of the new sentence
        return write_word(wctx, word_index, content);
    }
    ...
    if (!has_delimiter)
    {
        ...
        int result = insert_word_at_index(sentence, adjusted_index, content);
        if (result != SUCCESS)
        {
            log_message(ss_logger, LOG_ERROR, "Failed to insert word at index %d", adjusted_index);
            return result;
        }

        log_message(ss_logger, LOG_DEBUG, "Inserted word '%s' at adjusted index %d (original %d) in sentence %d", content, adjusted_index, word_index, wctx->sentence_index);
    }
    else
    {
        // Content has delimiter - need to split sentence

        // Special case: delimiter at start (e.g., ".sv" or standalone ".")
        if (delimiter_pos == 0)
        {
            // Set delimiter for current sentence WITHOUT inserting any word
            sentence->delimiter = delimiter;

            // Check if there's content after delimiter
            ...
            if (has_remaining_content)
            {
                ...
                log_message(ss_logger, LOG_INFO, "Delimiter at start, created new sentence for remaining content");

                // Update context - move to new sentence
                wctx->sentence_index = wctx->sentence_index + 1;

                // Key insight: the remaining content is part of the CURRENT word being processed
                // We need the remaining "skr" from ".skr" to go at position 0 of new sentence
                // But the NEXT word from client should go at position 1
                // So set offset to word_index (not word_index + 1) temporarily
                wctx->word_index_offset = word_index;

                // Process remaining content - this will insert at adjusted_index = word_index - word_index = 0
                int result = write_word(wctx, word_index, remaining);
                ...
                // Now adjust offset so next word from client (index word_index + 1)
                // maps to position 1 in the new sentence
                // Next word will have adjusted_index = (word_index + 1) - word_index = 1
                // DON'T change offset here! Keep it at word_index
                // This way: next word (index word_index+1) gets adjusted_index = (word_index+1) - word_index = 1

                return SUCCESS;
            }
            else
            {
                // Standalone delimiter - need to create new sentence for next words
                log_message(ss_logger, LOG_INFO, "Standalone delimiter '%c' for sentence %d",
                            delimiter, wctx->sentence_index);

                // Create new sentence for subsequent words
                SentenceNode *new_sentence = create_sentence_node(wctx->file->sentence_count);
                if (!new_sentence)
                {
                    log_message(ss_logger, LOG_ERROR, "Failed to create new sentence");
                ...
                wctx->sentence_index = wctx->sentence_index + 1;
                wctx->word_index_offset = word_index + 1;

                log_message(ss_logger, LOG_INFO, "Created new sentence %d after standalone delimiter",
                            wctx->sentence_index);
            }
        ...
        // Normal case: delimiter in middle or at end
        // Insert word before delimiter (if any text before delimiter)
        int actual_insert_pos = adjusted_index;
        if (delimiter_pos > 0)
        {
            ...
            int result = insert_word_at_index(sentence, adjusted_index, word_before);
            if (result != SUCCESS)
            {
                log_message(ss_logger, LOG_ERROR, "Failed to insert word before delimiter");
                return result;
            }
            actual_insert_pos = adjusted_index + 1; // Move split point after inserted word
        }
        ...
        if (split_point || has_remaining_content)
        {
            ...
            // Update the write context to point to the new sentence for subsequent writes
            wctx->sentence_index = wctx->sentence_index + 1;
            wctx->word_index_offset = word_index + 1; // Next word index from client should map to index 0 of new sentence

            // Process remaining content after delimiter (if any in the input)
            if (has_remaining_content)
            {
                // Recursively process remaining content into the new sentence
                int result = write_word(wctx, word_index + 1, remaining); // Use original index + 1 so offset calc works
...
int commit_write(FileOperationContext *ctx, WriteContext *wctx, const char *filepath)
{
    ...
    // Now release write lock for the ORIGINAL sentence (the one we locked in begin_write)
    // Note: wctx->sentence_index may have changed during the write due to sentence splits
    SentenceNode *sentence = get_sentence(wctx->file, wctx->original_sentence_index);
    if (sentence)
    {
    ...
    {
        release_sentence_lock(ctx, lock_path, wctx->original_sentence_index, wctx->username);
    }
...
void rollback_write(FileOperationContext *ctx, WriteContext *wctx, const char *filepath)
{
    ...
    // Release lock if held - use ORIGINAL sentence index
    if (wctx->file)
    {
        SentenceNode *sentence = get_sentence(wctx->file, wctx->original_sentence_index);
        if (sentence)
...
// Clear all sentence locks for a file (used after UNDO or major file changes)
void clear_all_sentence_locks_for_file(FileOperationContext *ctx, const char *filepath)
{
    if (!ctx || !filepath)
    {
        return;
    ...
    int removed = 0;
    for (int i = 0; i < ctx->sentence_lock_count;)
    {
        SentenceLockEntry *entry = &ctx->sentence_locks[i];
        if (strcmp(entry->filepath, full_path) == 0)
        {
    ...
    if (removed > 0)
    {
        log_message(ss_logger, LOG_INFO, "Cleared %d sentence locks for file: %s", removed, filepath);
    }
}
...
int undo_file(FileOperationContext *ctx, const char *filepath, const char *username)
{
    ...
    // Clear any lingering sentence locks for this file
    clear_all_sentence_locks_for_file(ctx, filepath);
