...
{
    FileStructure *file;
    int sentence_index;          // Current sentence being written to (changes during write)
    int original_sentence_index; // Original sentence index from BEGIN_WRITE (for lock release)
    char username[MAX_USERNAME_LEN];
    ...
    int word_index_offset;  // Offset to apply to incoming word indices after sentence splits
};
